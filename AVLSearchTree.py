class Node:
    ...
    def __init__(self, data, parent):
        ...
        self.left_node = None
        self.height = 0         # in contrast to regular BST AVL nodes have the height attribute
...
class AVLTree:
    ...
    def insert_node(self, data, node):
        ...
        self.handle_violation(node) # after every node insertion we have to handle possible violation of AVL order
    ...
    def handle_violation(self, node):
        ...
        while node is not None:
            ...
            self.violation_solver(node)
            node = node.parent

    def violation_solver(self, node):

        balance = self.calculate_balance(node)

        # Balance greater than 1 shows that tree is left heavy
        if balance > 1:

            # negative balance on the left node indicates left right heavy situation (left rotation on left child + right rotation on the node)
            if self.calculate_balance(node.left_node) < 0:
                self.rotate_left(node.left_node)

            self.rotate_right(node)

        # Balance smaller than 1 shows that tree is right heavy
        if balance < -1:

            # positive balance on the left node indicates right left heavy situation (right rotation on right child + left rotation on the node)            if self.calculate_balance(node.right_node) > 0:
